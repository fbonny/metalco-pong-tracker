...
export default function PlayerStatsModal({ player, onClose }: PlayerStatsModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in" onClick={onClose}>
      <div className="bg-background border-2 border-foreground max-w-lg w-full max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-6 space-y-4">
          <div>
            ...
            <div className="w-full h-4 border-2 border-foreground">
              <div className="h-full bg-foreground transition-all" style={{ width: `${winRate}%` }} />
            </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="p-4 border-2 border-destructive bg-destructive/5">
                  ...
                  {advancedStats.nemesis ? (
                    <>
                      <div className="font-bold text-sm truncate mb-1">{advancedStats.nemesis.opponentName}</div>
                      <div className="text-xs text-muted-foreground">{advancedStats.nemesis.wins}V - {advancedStats.nemesis.losses}S</div>
                    </>
                  ) : (
                ...
                <div className="p-4 border-2 border-green-600 bg-green-600/5">
                  ...
                  {advancedStats.victim ? (
                    <>
                      <div className="font-bold text-sm truncate mb-1">{advancedStats.victim.opponentName}</div>
                      <div className="text-xs text-muted-foreground">{advancedStats.victim.wins}V - {advancedStats.victim.losses}S</div>
                    </>
                  ) : (
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="p-4 border-2 border-foreground text-center">
                  ...
                  <div className="text-2xl font-bold">
                    ...
                    <span className={`text-sm ml-1 ${advancedStats.currentStreak.type === 'W' ? 'text-green-600' : 'text-red-600'}`}>
              ...
              <div className="p-4 border-2 border-foreground">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="text-center p-3 bg-muted/50">
                    ...
                    <div className="text-xl font-bold">{advancedStats.singoloWinRate.toFixed(0)}%</div>
                    <div className="text-xs text-muted-foreground mt-1">{advancedStats.singoloMatches} partite</div>
                  </div>
                  <div className="text-center p-3 bg-muted/50">
                    <div className="text-xs text-muted-foreground mb-1">Doppio</div>
                    <div className="text-xl font-bold">{advancedStats.doppioWinRate.toFixed(0)}%</div>
                    <div className="text-xs text-muted-foreground mt-1">{advancedStats.doppioMatches} partite</div>
                  </div>
                </div>
              </div>
