...
import { Player, updatePlayer, deletePlayer, getPlayers, recalculateAllStats, FameEntry, Match, getMatches } from '@/lib/database';
import { formatPoints } from '@/lib/formatUtils';
import { calculateAdvancedStats } from '@/lib/statsUtils';
import PlayerAvatar from '@/components/PlayerAvatar';
import { compressImage } from '@/lib/imageUtils';
import { X, Crown, Plus, TrendingUp, TrendingDown, Swords, Target, Flame } from 'lucide-react';
...
export default function PlayerProfileModal({ player, onClose, onUpdate }: PlayerProfileModalProps) {
  ...
  async function loadAdvancedData() {
    ...
    const rank = playersData.findIndex(p => p.id === player.id) + 1;
    setCurrentRank(rank);
              ...
              {advancedStats.recentForm.map((form, idx) => (
                <div key={idx} className="flex items-center justify-between text-sm">
                  ...
                  <span className="font-semibold">
                    ...
                    {form.pointsChange > 0 && (
                      <span className="text-green-600 ml-2">+{form.pointsChange}pts</span>
                    )}
                  </span>
                </div>
              ))}
